...
namespace souffle {
using namespace ram;
class Sf_x_1 : public SouffleProgram {
private:
...
// -- Table: r_bor
ram::Relation<Auto,3>* rel_3_r_bor;
souffle::RelationWrapper<1,ram::Relation<Auto,3>,Tuple<RamDomain,3>,3,false,true> wrapper_rel_3_r_bor;
public:
Sf_x_1() : rel_1_A(new ram::Relation<Auto,1>()),
wrapper_rel_1_A(*rel_1_A,symTable,"A",std::array<const char *,1>{"i:number"},std::array<const char *,1>{"x"}),
rel_3_r_bor(new ram::Relation<Auto,3>()),
wrapper_rel_3_r_bor(*rel_3_r_bor,symTable,"r_bor",std::array<const char *,3>{"i:number","i:number","i:number"},std::array<const char *,3>{"x","y","z"}){
addRelation("A",&wrapper_rel_1_A,1,0);
addRelation("r_bor",&wrapper_rel_3_r_bor,0,1);
}
~Sf_x_1() {
delete rel_1_A;
delete rel_3_r_bor;
...
SignalHandler::instance()->setMsg(R"_(r_bor((xbory),x,y) :- 
   A(x),
   A(y).
in file /home/lyndonhenry/Dropbox/workspace/souffle/tests/semantic/bitwise/bitwise.dl [17:1-17:36])_");
/* BEGIN visitInsert @RamExecutor.cpp:1287 */
if (!rel_1_A->empty()) {
auto part = rel_1_A->partition();
PARALLEL_START;
CREATE_OP_CONTEXT(rel_1_A_op_ctxt,rel_1_A->createContext());
CREATE_OP_CONTEXT(rel_3_r_bor_op_ctxt,rel_3_r_bor->createContext());
...
rel_3_r_bor->insert(tuple,READ_OP_CONTEXT(rel_3_r_bor_op_ctxt));
...
if (performIO) {
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"filename","/home/lyndonhenry/Dropbox/workspace/souffle/tests/testsuite.dir/semantic/bitwise/id/r_bor.csv"},{"name","r_bor"}});
if (!outputDirectory.empty() && directiveMap["IO"] == "file" && directiveMap["filename"].front() != '/') {directiveMap["filename"] = outputDirectory + "/" + directiveMap["filename"];}
IODirectives ioDirectives(directiveMap);
IOSystem::getInstance().getWriter(SymbolMask({0, 0, 0}), symTable, ioDirectives, 0)->writeAll(*rel_3_r_bor);
} catch (std::exception& e) {std::cerr << e.what();exit(1);}
}
/* END visitStore @RamExecutor.cpp:1283 */
/* BEGIN visitDrop @RamExecutor.cpp:1397 */
if (performIO || 0) rel_3_r_bor->purge();
/* END visitDrop @RamExecutor.cpp:1401 */
...
std::cout << "\n";
std::cout << "Relation rel_3_r_bor:\n";
rel_3_r_bor->printHintStatistics(std::cout,"  ");
std::cout << "\n";
}
}
public:
...
try {IODirectives ioDirectives;
ioDirectives.setIOType("stdout");
ioDirectives.setRelationName("rel_3_r_bor");
IOSystem::getInstance().getWriter(SymbolMask({0, 0, 0}), symTable, ioDirectives, 0)->writeAll(*rel_3_r_bor);
} catch (std::exception& e) {std::cerr << e.what();exit(1);}
}
public:
...
SouffleProgram *newInstance_x(){return new Sf_x_1;}
SymbolTable *getST_x(SouffleProgram *p){return &reinterpret_cast<Sf_x_1*>(p)->symTable;}
#ifdef __EMBEDDED_SOUFFLE__
class factory_Sf_x_1: public souffle::ProgramFactory {
SouffleProgram *newInstance() {
return new Sf_x_1();
};
public:
factory_Sf_x_1() : ProgramFactory("x"){}
};
static factory_Sf_x_1 __factory_Sf_x_1_instance;
}
#else
}
