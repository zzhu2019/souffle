...
const Component* ComponentLookup::getComponent(
    ...
    TypeIdentifier boundName = activeBinding.find(name);
    if (boundName.empty()) {
...
struct ComponentContent {
    ...
    void add(std::unique_ptr<Type>& type, ErrorReport& report) {
        // add to result content (check existence first)
        auto foundItem =
                std::find_if(types.begin(), types.end(), [&](const std::unique_ptr<Type>& element) {
    ...
    void add(std::unique_ptr<Relation>& rel, ErrorReport& report) {
        // add to result content (check existence first)
        auto foundItem = std::find_if(
                relations.begin(), relations.end(), [&](const std::unique_ptr<Relation>& element) {
    ...
    void add(std::unique_ptr<IODirective>& io, ErrorReport& report) {
        ...
        auto foundItem = std::find_if(ioDirectives.begin(), ioDirectives.end(),
                [&](const std::unique_ptr<IODirective>& element) {
 ...
 */
ComponentContent getInstantiatedContent(const ComponentInit& componentInit,
        const Component* enclosingComponent, const ComponentLookup& componentLookup,
        std::vector<std::unique_ptr<Clause>>& orphans, ErrorReport& report,
 ...
 */
void collectContent(const Component& component, const TypeBinding& binding,
        const Component* enclosingComponent, const ComponentLookup& componentLookup, ComponentContent& res,
        std::vector<std::unique_ptr<Clause>>& orphans, std::set<std::string> overridden,
    ...
    for (const auto& cur : component.getTypes()) {
        // create a clone
        std::unique_ptr<Type> type(cur->clone());

        // instantiate elements of union types
        visitDepthFirst(*type, [&](const UnionType& type) {
            for (const auto& name : type.getTypes()) {
                TypeIdentifier newName = binding.find(name);
                if (!newName.empty()) {
                    const_cast<TypeIdentifier&>(name) = newName;
                }
            }
        });

        // instantiate elements of record types
        visitDepthFirst(*type, [&](const RecordType& type) {
            for (const auto& field : type.getFields()) {
                TypeIdentifier newName = binding.find(field.type);
                if (!newName.empty()) {
                    const_cast<TypeIdentifier&>(field.type) = newName;
                }
            }
        });
    ...
    for (const auto& cur : component.getRelations()) {
        // create a clone
        std::unique_ptr<Relation> rel(cur->clone());

        // update attribute types
        for (Attribute* attr : rel->getAttributes()) {
            TypeIdentifier forward = binding.find(attr->getTypeName());
            if (!forward.empty()) {
    ...
    for (const auto& cur : component.getIODirectives()) {
        // create a clone
        std::unique_ptr<IODirective> io(cur->clone());

        res.add(io, report);
    }

    // index the available relations
    std::map<RelationIdentifier, Relation*> index;
    ...
    for (const auto& cur : component.getClauses()) {
        if (overridden.count(cur->getHead()->getName().getNames()[0]) == 0) {
            Relation* rel = index[cur->getHead()->getName()];
            if (rel) {
                rel->addClause(std::unique_ptr<Clause>(cur->clone()));
            } else {
                orphans.push_back(std::unique_ptr<Clause>(cur->clone()));
            }
        }
    }
...
ComponentContent getInstantiatedContent(const ComponentInit& componentInit,
        const Component* enclosingComponent, const ComponentLookup& componentLookup,
        std::vector<std::unique_ptr<Clause>>& orphans, ErrorReport& report, const TypeBinding& binding,
    ...
    // update type names
    std::map<TypeIdentifier, TypeIdentifier> typeNameMapping;
    ...
    std::map<RelationIdentifier, RelationIdentifier> relationNameMapping;
    for (const auto& cur : res.relations) {
        auto newName = componentInit.getInstanceName() + cur->getName();
    ...
    auto fixNames = [&](const Node& node) {
        ...
        visitDepthFirst(node, [&](const IODirective& ioDirective) {
            auto pos = relationNameMapping.find(ioDirective.getName());
            if (pos != relationNameMapping.end()) {
                const_cast<IODirective&>(ioDirective).setName(pos->second);
            }
        });
    };
...
bool ComponentInstantiationTransformer::transform(TranslationUnit& translationUnit) {
    ...
    for (const auto& cur : program.instantiations) {
        std::vector<std::unique_ptr<Clause>> orphans;
