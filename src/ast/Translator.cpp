...
SymbolMask getSymbolMask(const Relation& rel, const TypeEnvironment& typeEnv) {
 ...
 */
std::string getRelationName(const RelationIdentifier& id) {
    return toString(join(id.getNames(), "-"));
}

IODirectives getInputIODirectives(
        const Relation* rel, std::string filePath = std::string(), std::string fileExt = std::string()) {
...
std::vector<IODirectives> getOutputIODirectives(const Relation* rel, const TypeEnvironment* typeEnv,
...
std::unique_ptr<RamRelation> getRamRelation(const Relation* rel, const TypeEnvironment* typeEnv,
...
std::string Translator::translateRelationName(const RelationIdentifier& id) {
...
class ValueIndex {
     ...
     */
    typedef std::map<const RecordInit*, Location> record_definition_map;

    /**
     * The type mapping record init expressions to the loop level where
     * they get unpacked.
     */
    typedef std::map<const RecordInit*, int> record_unpack_map;

    /**
     * A map from Aggregators to storage locations. Note, since in this case
     * Aggregators are indexed by their values (not their address) no standard
     * map can be utilized.
     */
    typedef std::vector<std::pair<const Aggregator*, Location>> aggregator_location_map;
...
public:
    ...
    void addVarReference(const Variable& var, const Location& l) {
        std::set<Location>& locs = var_references[var.getName()];
        locs.insert(l);
    }

    void addVarReference(const Variable& var, int level, int pos, const std::string& name = "") {
    ...
    const Location& getDefinitionPoint(const Variable& var) const {
    ...
    void setRecordDefinition(const RecordInit& init, const Location& l) {
        record_definitions[&init] = l;
    }

    void setRecordDefinition(const RecordInit& init, int level, int pos, std::string name = "") {
        setRecordDefinition(init, Location({level, pos, name}));
    }

    const Location& getDefinitionPoint(const RecordInit& init) const {
    ...
    void setRecordUnpackLevel(const RecordInit& init, int level) {
        record_unpacks[&init] = level;
    }

    int getRecordUnpackLevel(const RecordInit& init) const {
    ...
    void setAggregatorLocation(const Aggregator& agg, const Location& loc) {
        aggregator_locations.push_back(std::make_pair(&agg, loc));
    }

    const Location& getAggregatorLocation(const Aggregator& agg) const {
        // search list
...
std::unique_ptr<RamValue> translateValue(const Argument* arg, const ValueIndex& index = ValueIndex()) {
    std::unique_ptr<RamValue> val;
    if (!arg) {
        return val;
    }

    if (const Variable* var = dynamic_cast<const Variable*>(arg)) {
    ...
    } else if (const Constant* c = dynamic_cast<const Constant*>(arg)) {
        val = std::make_unique<RamNumber>(c->getIndex());
    } else if (const UnaryFunctor* uf = dynamic_cast<const UnaryFunctor*>(arg)) {
        val = std::unique_ptr<RamValue>(
                new RamUnaryOperator(uf->getFunction(), translateValue(uf->getOperand(), index)));
    } else if (const BinaryFunctor* bf = dynamic_cast<const BinaryFunctor*>(arg)) {
        val = std::make_unique<RamBinaryOperator>(
                bf->getFunction(), translateValue(bf->getLHS(), index), translateValue(bf->getRHS(), index));
    } else if (const TernaryFunctor* tf = dynamic_cast<const TernaryFunctor*>(arg)) {
        val = std::unique_ptr<RamValue>(
    ...
    } else if (dynamic_cast<const Counter*>(arg)) {
        val = std::make_unique<RamAutoIncrement>();
    } else if (const RecordInit* init = dynamic_cast<const RecordInit*>(arg)) {
        std::vector<std::unique_ptr<RamValue>> values;
        for (const auto& cur : init->getArguments()) {
            values.push_back(translateValue(cur, index));
        }
        val = std::make_unique<RamPack>(std::move(values));
    } else if (const Aggregator* agg = dynamic_cast<const Aggregator*>(arg)) {
        // here we look up the location the aggregation result gets bound
        auto loc = index.getAggregatorLocation(*agg);
        val = std::make_unique<RamElementAccess>(loc.level, loc.component, loc.name);
    } else if (const SubroutineArgument* subArg = dynamic_cast<const SubroutineArgument*>(arg)) {
...
std::unique_ptr<RamStatement> Translator::translateClause(const Clause& clause, const Program* program,
        const TypeEnvironment* typeEnv, int version, bool ret, bool hashset) {
    ...
    if (clause.getExecutionPlan() && clause.getExecutionPlan()->hasOrderFor(version)) {
        ...
        const ExecutionOrder& order = clause.getExecutionPlan()->getOrderFor(version);

        // create a copy and fix order
        std::unique_ptr<Clause> copy(clause.clone());
    ...
    const Atom& head = *clause.getHead();

    // a utility to translate atoms to relations
    auto getRelation = [&](const Atom* atom) {
    ...
    std::vector<const Node*> op_nesting;
    ...
    for (Atom* atom : clause.getAtoms()) {
        ...
        std::map<const Node*, std::unique_ptr<arg_list>> nodeArgs;

        std::function<arg_list*(const Node*)> getArgList = [&](const Node* curNode) {
            if (!nodeArgs.count(curNode)) {
                if (auto rec = dynamic_cast<const RecordInit*>(curNode)) {
                    nodeArgs.insert(std::make_pair(curNode, std::make_unique<arg_list>(rec->getArguments())));
                } else if (auto atom = dynamic_cast<const Atom*>(curNode)) {
                    nodeArgs.insert(
        ...
        std::function<void(const Node*)> indexValues = [&](const Node* curNode) {
            ...
            for (size_t pos = 0; pos < cur->size(); pos++) {
                ...
                if (auto rec = dynamic_cast<const RecordInit*>(arg)) {
    ...
    std::vector<const Aggregator*> aggregators;
    visitDepthFirstPostOrder(clause, [&](const Aggregator& cur) {

        // add each aggregator expression only once
        if (any_of(aggregators, [&](const Aggregator* agg) { return *agg == cur; })) {
            return;
        }
        ...
        assert(dynamic_cast<const Atom*>(cur.getBodyLiterals()[0]));
        const Atom& atom = static_cast<const Atom&>(*cur.getBodyLiterals()[0]);
        for (size_t pos = 0; pos < atom.getArguments().size(); ++pos) {
            if (const Variable* var = dynamic_cast<const Variable*>(atom.getArgument(pos))) {
    ...
    if (ret) {
        ...
        for (Argument* arg : head.getArguments()) {
    ...
    for (auto it = aggregators.rbegin(); it != aggregators.rend(); ++it) {
        ...
        const Atom* atom = dynamic_cast<const Atom*>(cur->getBodyLiterals()[0]);
        ...
        for (size_t pos = 0; pos < atom->argSize(); ++pos) {
            if (Constant* c = dynamic_cast<Constant*>(atom->getArgument(pos))) {
    ...
    while (!op_nesting.empty()) {
        ...
        if (const Atom* atom = dynamic_cast<const Atom*>(cur)) {
            ...
            for (size_t pos = 0; pos < atom->argSize(); ++pos) {
                if (Constant* c = dynamic_cast<Constant*>(atom->getArgument(pos))) {
                    op->addCondition(std::unique_ptr<RamCondition>(new RamBinaryRelation(
                            ...
                            std::make_unique<RamNumber>(c->getIndex()))));
                } else if (Aggregator* agg = dynamic_cast<Aggregator*>(atom->getArgument(pos))) {
        ...
        } else if (const RecordInit* rec = dynamic_cast<const RecordInit*>(cur)) {
            // add an unpack level
            ...
            for (size_t pos = 0; pos < rec->getArguments().size(); ++pos) {
                if (Constant* c = dynamic_cast<Constant*>(rec->getArguments()[pos])) {
                    op->addCondition(std::unique_ptr<RamCondition>(new RamBinaryRelation(
    ...
    for (const auto& lit : clause.getBodyLiterals()) {
        // for atoms
        if (dynamic_cast<const Atom*>(lit)) {
            // covered already within the scan/lookup generation step

            // for binary relations
        } else if (auto binRel = dynamic_cast<const BinaryConstraint*>(lit)) {
            ...
            // for negations
        } else if (auto neg = dynamic_cast<const Negation*>(lit)) {
            // get contained atom
            const Atom* atom = neg->getAtom();
...
std::unique_ptr<RamStatement> Translator::translateNonRecursiveRelation(const Relation& rel,
 ...
 */
void nameUnnamedVariables(Clause* clause) {
    // the node mapper conducting the actual renaming
    struct Instantiator : public NodeMapper {
        mutable int counter;
        ...
        std::unique_ptr<Node> operator()(std::unique_ptr<Node> node) const override {
            ...
            if (dynamic_cast<UnnamedVariable*>(node.get())) {
...
std::unique_ptr<RamStatement> Translator::translateRecursiveRelation(const std::set<const Relation*>& scc,
        const Program* program, const RecursiveClauses* recursiveClauses, const TypeEnvironment& typeEnv) {
    // initialize sections
    ...
    std::map<const Relation*, std::unique_ptr<RamRelation>> rrel;
    std::map<const Relation*, std::unique_ptr<RamRelation>> relDelta;
    std::map<const Relation*, std::unique_ptr<RamRelation>> relNew;
    ...
    auto isInSameSCC = [&](
            const Relation* rel) { return std::find(scc.begin(), scc.end(), rel) != scc.end(); };
    ...
    for (const Relation* rel : scc) {
        ...
        for (size_t i = 0; i < rel->clauseSize(); i++) {
            ...
            for (size_t j = 0; j < atoms.size(); ++j) {
                ...
                const Relation* atomRelation = getAtomRelation(atom, program);
                ...
                std::unique_ptr<Clause> r1(cl->clone());
                r1->getHead()->setName(relNew[rel]->getName());
                r1->getAtoms()[j]->setName(relDelta[atomRelation]->getName());
                r1->addToBody(std::unique_ptr<Literal>(
                        new Negation(std::unique_ptr<Atom>(cl->getHead()->clone()))));
                ...
                for (size_t k = j + 1; k < atoms.size(); k++) {
                    if (isInSameSCC(getAtomRelation(atoms[k], program))) {
                        Atom* cur = r1->getAtoms()[k]->clone();
                        cur->setName(relDelta[getAtomRelation(atoms[k], program)]->getName());
                        r1->addToBody(std::make_unique<Negation>(std::unique_ptr<Atom>(cur)));
                    }
                }
...
void createAndLoad(std::unique_ptr<RamStatement>& current, const Relation* rel,
    ...
    Relation* mrel = const_cast<Relation*>(rel)->clone();
    if ((!rel->isInput() && !loadInputOnly) ||
            (!isComputed && !isRecursive && !loadInputOnly && rel->isInput())) {
        std::unique_ptr<IODirective> directive = std::make_unique<IODirective>();
...
void printSizeStore(std::unique_ptr<RamStatement>& current, const Relation* rel,
...
std::unique_ptr<RamStatement> Translator::makeSubproofSubroutine(
        const Clause& clause, const Program* program, const TypeEnvironment& typeEnv) {
    // make intermediate clause with constraints
    std::unique_ptr<Clause> intermediateClause(clause.clone());

    // name unnamed variables
    nameUnnamedVariables(intermediateClause.get());

    // add constraint for each argument in head of atom
    Atom* head = intermediateClause->getHead();
    for (size_t i = 0; i < head->getArguments().size() - 2; i++) {
        auto arg = head->getArgument(i);

        if (auto var = dynamic_cast<Variable*>(arg)) {
            intermediateClause->addToBody(std::make_unique<BinaryConstraint>(BinaryConstraintOp::EQ,
                    std::unique_ptr<Argument>(var->clone()), std::make_unique<SubroutineArgument>(i)));
        } else if (auto func = dynamic_cast<Functor*>(arg)) {
            intermediateClause->addToBody(std::make_unique<BinaryConstraint>(BinaryConstraintOp::EQ,
                    std::unique_ptr<Argument>(func->clone()), std::make_unique<SubroutineArgument>(i)));
        } else if (auto rec = dynamic_cast<RecordInit*>(arg)) {
            intermediateClause->addToBody(std::make_unique<BinaryConstraint>(BinaryConstraintOp::EQ,
                    std::unique_ptr<Argument>(rec->clone()), std::make_unique<SubroutineArgument>(i)));
        }
    }
    ...
    for (size_t i = 0; i < intermediateClause->getBodyLiterals().size(); i++) {
        ...
        if (auto atom = dynamic_cast<Atom*>(lit)) {
            ...
            intermediateClause->addToBody(std::make_unique<BinaryConstraint>(BinaryConstraintOp::LT,
                    std::unique_ptr<Argument>(atom->getArgument(arity - 1)->clone()),
                    std::make_unique<SubroutineArgument>(levelIndex)));
        }
    }
...
std::unique_ptr<RamProgram> Translator::translateProgram(const TranslationUnit& translationUnit) {
    ...
    for (const RelationScheduleStep& step : schedule) {
        ...
        for (const Relation* rel : step.computed()) {
        ...
        if (!step.recursive()) {
            ...
            const Relation* rel = *step.computed().begin();
        ...
        if (!Global::config().has("provenance")) {
            for (const Relation* rel : step.expired()) {
                appendStmt(current,
    ...
    if (Global::config().has("provenance")) {
        visitDepthFirst(translationUnit.getProgram()->getRelations(), [&](const Clause& clause) {
...
std::unique_ptr<RamTranslationUnit> Translator::translateUnit(TranslationUnit& tu) {
