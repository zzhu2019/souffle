...
private:
    ...
    static void removeComplexTermsInAtoms(Clause& clause);
...
public:
     ...
     */
    static std::unique_ptr<Clause> resolveAliases(const Clause& clause);
     ...
     */
    static std::unique_ptr<Clause> removeTrivialEquality(const Clause& clause);
     ...
     */
    static void resolveAliases(Program& program);
};
...
public:
     ...
     */
    static bool removeRelationCopies(Program& program);
};
...
private:
     ...
     */
    static void removeEmptyRelationUses(TranslationUnit& translationUnit, Relation* emptyRelation);
 ...
 *      - newrel() :- c(y), d(y).
 */
class ExtractDisconnectedLiteralsTransformer : public Transformer {
private:
    bool transform(TranslationUnit& translationUnit) override;
 ...
 */
class MagicSetTransformer : public Transformer {
private:
    bool transform(TranslationUnit& translationUnit) override;
...
private:
    std::vector<std::unique_ptr<Transformer>> pipeline;
    bool transform(TranslationUnit& translationUnit) override;

public:
    template <typename... Args>
    PipelineTransformer(Args... args) {
        std::unique_ptr<Transformer> tmp[] = {std::move(args)...};
        for (auto& cur : tmp) {
...
public:
    ConditionalTransformer(std::function<bool()> cond, std::unique_ptr<Transformer> transformer)
            : condition(cond), transformer(std::move(transformer)) {}

    ConditionalTransformer(bool cond, std::unique_ptr<Transformer> transformer)
