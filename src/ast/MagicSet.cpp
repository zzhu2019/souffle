...
bool isEqualAdornment(
        RelationIdentifier pred1, std::string adorn1, RelationIdentifier pred2, std::string adorn2) {
...
bool contains(std::set<AdornedPredicate> adornedPredicates, RelationIdentifier atomName,
...
bool isAggRel(RelationIdentifier rel) {
...
std::string getString(const Argument* arg) {
...
bool isBoundComposite(const Variable* compositeVariable, std::set<std::string> boundArgs,
...
bool isBoundArgument(Argument* arg, std::set<std::string> boundArgs, BindingStore& compositeBindings) {
...
bool hasBoundArgument(Atom* atom, std::set<std::string> boundArgs, BindingStore& compositeBindings) {
...
bool isBindingConstraint(Argument* lhs, Argument* rhs, std::set<std::string> boundArgs) {
    std::string lhs_name = getString(lhs);
    std::string rhs_name = getString(rhs);

    // only want to check variables we have not bound yet
    if (dynamic_cast<Variable*>(lhs) && (boundArgs.find(lhs_name) == boundArgs.end())) {
        // return true if the rhs is a bound variable or a constant
        if (dynamic_cast<Variable*>(rhs) && (boundArgs.find(rhs_name) != boundArgs.end())) {
            return true;
        } else if (dynamic_cast<Constant*>(rhs)) {
            return true;
        }
    }
    return false;
...
bool containsAggregators(Clause* clause) {
    bool found = false;

    // check for aggregators
    visitDepthFirst(*clause, [&](const Aggregator& aggr) { found = true; });
...
std::string getNextEdbName(Program* program) {
    static int edbNum = 0;
...
void updateQualifier(Relation* originalRelation, Relation* newRelation) {
...
Relation* createNewRelation(Relation* original, RelationIdentifier newName) {
    ...
    for (Attribute* attr : original->getAttributes()) {
        newRelation->addAttribute(std::unique_ptr<Attribute>(attr->clone()));
    }
...
RelationIdentifier createMagicIdentifier(RelationIdentifier relationName, size_t outputNumber) {
    ...
    RelationIdentifier newRelationName(newMainName.str());
...
RelationIdentifier createAdornedIdentifier(RelationIdentifier relationName, std::string adornment) {
    std::vector<std::string> relationNames = relationName.getNames();
...
RelationIdentifier createSubIdentifier(RelationIdentifier relationName, size_t start, size_t length) {
    std::vector<std::string> relationNames = relationName.getNames();
...
std::set<RelationIdentifier> addAggregators(
        Clause* clause, std::set<RelationIdentifier> ignoredNames) {
    std::set<RelationIdentifier> retVal = ignoredNames;

    visitDepthFirst(*clause, [&](const Aggregator& aggregator) {
        visitDepthFirst(aggregator, [&](const Atom& atom) { retVal.insert(atom.getName()); });
    });
...
std::set<RelationIdentifier> addBackwardDependencies(
        const Program* program, std::set<RelationIdentifier> relations) {
    bool relationsAdded = false;
    std::set<RelationIdentifier> result;

    for (RelationIdentifier relName : relations) {
    ...
    for (Relation* rel : program->getRelations()) {
        for (Clause* clause : rel->getClauses()) {
            RelationIdentifier clauseHeadName = clause->getHead()->getName();
            if (!contains(relations, clauseHeadName)) {
                // Clause hasn't been added yet, so check if it needs to be added
                visitDepthFirst(*clause, [&](const Atom& subatom) {
                    RelationIdentifier atomName = subatom.getName();
...
std::set<RelationIdentifier> addForwardDependencies(
        const Program* program, std::set<RelationIdentifier> relations) {
    bool relationsAdded = false;
    std::set<RelationIdentifier> result;
    ...
    for (RelationIdentifier relName : relations) {
        ...
        Relation* associatedRelation = program->getRelation(relName);
        for (Clause* clause : associatedRelation->getClauses()) {
            visitDepthFirst(*clause, [&](const Atom& subatom) {
                RelationIdentifier atomName = subatom.getName();
...
std::set<RelationIdentifier> addIgnoredRelations(
        const Program* program, std::set<RelationIdentifier> relations) {
    ...
    if (contains(specifiedRelations, std::string("*"))) {
        return relations;
    }

    // find all specified relations
    std::set<RelationIdentifier> targetRelations;
    for (Relation* rel : program->getRelations()) {
    ...
    std::set<RelationIdentifier> retVal(relations);
    for (Relation* rel : program->getRelations()) {
...
std::pair<std::string, std::set<std::string>> bindArguments(
        Atom* currAtom, std::set<std::string> boundArgs, BindingStore& compositeBindings) {
    std::set<std::string> newlyBoundArgs;
    std::string atomAdornment = "";

    for (Argument* arg : currAtom->getArguments()) {
        if (isBoundArgument(arg, boundArgs, compositeBindings)) {
...
int getNextAtomNaiveSIPS(std::vector<Atom*> atoms, std::set<std::string> boundArgs,
        std::set<RelationIdentifier> edb, BindingStore& compositeBindings) {
    // find the first available atom with at least one bound argument
    int firstedb = -1;
    int firstidb = -1;
    for (size_t i = 0; i < atoms.size(); i++) {
        Atom* currAtom = atoms[i];
        if (currAtom == nullptr) {
            // already done - move on
            continue;
        }

        RelationIdentifier atomName = currAtom->getName();
...
int getNextAtomMaxBoundSIPS(std::vector<Atom*>& atoms, std::set<std::string> boundArgs,
        std::set<RelationIdentifier> edb, BindingStore& compositeBindings) {
    int maxBound = -1;
    int maxIndex = 0;
...
int getNextAtomMaxRatioSIPS(std::vector<Atom*>& atoms, std::set<std::string> boundArgs,
        std::set<RelationIdentifier> edb, BindingStore& compositeBindings) {
    double maxRatio = -1;
    int maxIndex = 0;
...
int getNextAtomSIPS(std::vector<Atom*>& atoms, std::set<std::string> boundArgs,
        std::set<RelationIdentifier> edb, BindingStore& compositeBindings) {
...
BindingStore bindComposites(const Program* program) {
    struct M : public NodeMapper {
        BindingStore& compositeBindings;
        std::set<BinaryConstraint*>& constraints;
        mutable int changeCount;

        M(BindingStore& compositeBindings, std::set<BinaryConstraint*>& constraints, int changeCount)
        ...
        std::unique_ptr<Node> operator()(std::unique_ptr<Node> node) const override {
            if (Functor* functor = dynamic_cast<Functor*>(node.get())) {
                // functor found
                changeCount++;
                ...
                constraints.insert(new BinaryConstraint(BinaryConstraintOp::EQ,
                        ...
                        std::unique_ptr<Argument>(functor->clone())));

                // update functor to be the variable created
                return std::move(newVariable);
            } else if (RecordInit* record = dynamic_cast<RecordInit*>(node.get())) {
                // record found
                changeCount++;
                ...
                constraints.insert(new BinaryConstraint(BinaryConstraintOp::EQ,
                        std::unique_ptr<Argument>(newVariable->clone()),
                        std::unique_ptr<Argument>(record->clone())));
...
void Adornment::run(const TranslationUnit& translationUnit) {
    // -------------
    // --- Setup ---
    // -------------
    ...
    std::vector<RelationIdentifier> outputQueries;
    ...
    for (Relation* rel : program->getRelations()) {
        RelationIdentifier relName = rel->getName();
    ...
    visitDepthFirst(*program,
            [&](const Negation& negation) { negatedAtoms.insert(negation.getAtom()->getName()); });
    ...
    for (size_t querynum = 0; querynum < outputQueries.size(); querynum++) {
        RelationIdentifier outputQuery = outputQueries[querynum];
        ...
        while (!currentPredicates.empty()) {
            ...
            Relation* rel = program->getRelation(currPredicate.getName());
            for (Clause* clause : rel->getClauses()) {
                if (clause->isFact()) {
                    continue;
                }
                ...
                Atom* clauseHead = clause->getHead();
                std::string headAdornment = currPredicate.getAdornment();
                std::vector<Argument*> headArguments = clauseHead->getArguments();
                ...
                std::vector<BinaryConstraint*> constraints = clause->getBinaryConstraints();
                for (BinaryConstraint* constraint : constraints) {
                    ...
                    Argument* lhs = constraint->getLHS();
                    Argument* rhs = constraint->getRHS();
                ...
                std::vector<Atom*> atoms = clause->getAtoms();
                int atomsAdorned = 0;
...
void separateDBs(Program* program) {
    for (Relation* relation : program->getRelations()) {
        RelationIdentifier relName = relation->getName();
        ...
        if (is_edb && is_idb) {
            ...
            Relation* newEdbRel = createNewRelation(relation, newEdbName);
            program->appendRelation(std::unique_ptr<Relation>(newEdbRel));

            // find all facts for the relation
            for (Clause* clause : relation->getClauses()) {
                if (clause->isFact()) {
                    // clause is fact - add it to the new EDB relation
                    Clause* newEdbClause = clause->clone();
                    newEdbClause->getHead()->setName(newEdbName);
                    program->appendClause(std::unique_ptr<Clause>(newEdbClause));
                }
            }

            // add a rule to the old relation that relates it to the new relation
            Clause* newIdbClause = new Clause();
            newIdbClause->setSrcLoc(nextSrcLoc(relation->getSrcLoc()));

            // oldname(arg1...argn) :- newname(arg1...argn)
            Atom* headAtom = new Atom(relName);
            Atom* bodyAtom = new Atom(newEdbName);
            ...
            for (size_t j = 0; j < numargs; j++) {
                ...
                argName.str("");
                argName << "arg" << j;
                headAtom->addArgument(std::make_unique<Variable>(argName.str()));
                bodyAtom->addArgument(std::make_unique<Variable>(argName.str()));
            }

            newIdbClause->setHead(std::unique_ptr<Atom>(headAtom));
            newIdbClause->addToBody(std::unique_ptr<Atom>(bodyAtom));

            program->appendClause(std::unique_ptr<Clause>(newIdbClause));
        }
    }
}

// returns the adornment of an (adorned) magic identifier
std::string extractAdornment(RelationIdentifier magicRelationName) {
...
Argument* extractConstant(SymbolTable& symbolTable, std::string normalisedConstant) {
    ...
    if (indicatorChar == 's') {
        // string argument
        return new StringConstant(symbolTable, stringRep.c_str());
    } else if (indicatorChar == 'n') {
        // numeric argument
        return new NumberConstant(stoi(stringRep));
    } else {
        // invalid format
        return nullptr;
    }
...
bool MagicSetTransformer::transform(TranslationUnit& translationUnit) {
    ...
    std::set<RelationIdentifier> negatedAtoms = adornment->getNegatedAtoms();
    std::set<RelationIdentifier> ignoredAtoms = adornment->getIgnoredAtoms();
    std::set<RelationIdentifier> oldIdb = adornment->getIDB();
    std::set<RelationIdentifier> newIdb;

    // additions
    std::vector<RelationIdentifier> newQueryNames;
    std::vector<Clause*> newClauses;

    // output handling
    std::vector<RelationIdentifier> outputQueries = adornment->getRelations();
    std::set<RelationIdentifier> addAsOutput;
    std::set<RelationIdentifier> addAsPrintSize;
    std::map<RelationIdentifier, std::vector<IODirective*>> outputDirectives;

    // ignore negated atoms
    for (RelationIdentifier relation : negatedAtoms) {
    ...
    for (size_t querynum = 0; querynum < outputQueries.size(); querynum++) {
        ...
        Relation* originalOutputRelation = program->getRelation(outputQuery);

        // add a relation for the output query
        // mN_outputname_ff...f()
        Relation* magicOutputRelation = new Relation();
        std::string frepeat = std::string(originalOutputRelation->getArity(), 'f');
        RelationIdentifier magicOutputName =
        ...
        program->appendRelation(std::unique_ptr<Relation>(magicOutputRelation));

        // add an empty fact to the program
        // i.e. mN_outputname_ff...f().
        Clause* outputFact = new Clause();
        outputFact->setSrcLoc(nextSrcLoc(originalOutputRelation->getSrcLoc()));
        outputFact->setHead(std::make_unique<Atom>(magicOutputName));
        program->appendClause(std::unique_ptr<Clause>(outputFact));
        ...
        for (AdornedClause adornedClause : adornedClauses) {
            Clause* clause = adornedClause.getClause();
            RelationIdentifier originalName = clause->getHead()->getName();
            ...
            RelationIdentifier newRelName = createAdornedIdentifier(originalName, headAdornment);
            Relation* adornedRelation = program->getRelation(newRelName);
            ...
            if (adornedRelation == nullptr) {
                ...
                Relation* originalRelation = program->getRelation(originalName);
                Relation* newRelation = createNewRelation(originalRelation, newRelName);
                ...
                if (originalRelation->isInput()) {
                    ...
                    newRelation->addIODirectives(std::unique_ptr<IODirective>(newDirective));
                }

                // add the created adorned relation to the program
                program->appendRelation(std::unique_ptr<Relation>(newRelation));
            ...
            int atomsSeen = 0;
            for (Literal* lit : newClause->getBodyLiterals()) {
                if (dynamic_cast<Atom*>(lit)) {
                    Atom* bodyAtom = dynamic_cast<Atom*>(lit);
                    RelationIdentifier atomName = bodyAtom->getName();
                    ...
                    if (contains(oldIdb, atomName)) {
                        if (!contains(ignoredAtoms, atomName)) {
                            ...
                            RelationIdentifier newAtomName =
            ...
            // -- -- For each IDB literal A_i in the body of C
            // -- -- -- Add mag(Ai^ai) :- mag(A^a), A1^a1, ..., Ai-1^ai-1 to the program
            std::vector<Literal*> body = newClause->getBodyLiterals();
            for (size_t i = 0; i < body.size(); i++) {
                Literal* currentLiteral = body[i];

                // only care about atoms in the body
                if (dynamic_cast<Atom*>(currentLiteral)) {
                    Atom* atom = dynamic_cast<Atom*>(currentLiteral);
                    RelationIdentifier atomName = atom->getName();
                    ...
                    if (contains(newIdb, atomName) && !contains(ignoredAtoms, atomName)) {
                        ...
                        RelationIdentifier newAtomName = createMagicIdentifier(atomName, querynum);

                        // if the magic version does not exist, create it
                        if (program->getRelation(newAtomName) == nullptr) {
                            Relation* magicRelation = new Relation();
                            ...
                            RelationIdentifier originalRelationName = createSubIdentifier(
                                    atomName, 0, endpt - 1);  // get rid of the extra + at the end
                            Relation* originalRelation = program->getRelation(originalRelationName);

                            // copy over the (bound) attributes from the original relation
                            int argcount = 0;
                            for (Attribute* attr : originalRelation->getAttributes()) {
                                if (currAdornment[argcount] == 'b') {
                                    magicRelation->addAttribute(std::unique_ptr<Attribute>(attr->clone()));
                                }
                                argcount++;
                            }
                            ...
                            program->appendRelation(std::unique_ptr<Relation>(magicRelation));
                        }

                        // start setting up the magic rule
                        Clause* magicClause = new Clause();
                        magicClause->setSrcLoc(nextSrcLoc(atom->getSrcLoc()));

                        // create the head of the magic rule
                        Atom* magicHead = new Atom(newAtomName);

                        // copy over (bound) arguments from the original atom
                        int argCount = 0;
                        for (Argument* arg : atom->getArguments()) {
                            if (currAdornment[argCount] == 'b') {
                                magicHead->addArgument(std::unique_ptr<Argument>(arg->clone()));
                            }
                            argCount++;
                        }

                        // head complete!
                        magicClause->setHead(std::unique_ptr<Atom>(magicHead));

                        // -- create the body --
                        // create the first body argument (mag(origClauseHead^adornment))
                        RelationIdentifier magPredName =
                                createMagicIdentifier(newClause->getHead()->getName(), querynum);
                        Atom* addedMagicPred = new Atom(magPredName);
                        ...
                        if (program->getRelation(magPredName) == nullptr) {
                            ...
                            Relation* newMagicRelation =
                                    createMagicRelation(originalRelation, magPredName);

                            // add the new relation to the prgoram
                            program->appendRelation(std::unique_ptr<Relation>(newMagicRelation));
                        }

                        // add (bound) arguments to the magic predicate from the clause head
                        argCount = 0;
                        for (Argument* arg : newClause->getHead()->getArguments()) {
                            if (headAdornment[argCount] == 'b') {
                                addedMagicPred->addArgument(std::unique_ptr<Argument>(arg->clone()));
                            }
                            argCount++;
                        }

                        // first argument complete!
                        magicClause->addToBody(std::unique_ptr<Atom>(addedMagicPred));

                        // add the rest of the necessary arguments
                        for (size_t j = 0; j < i; j++) {
                            magicClause->addToBody(std::unique_ptr<Literal>(body[j]->clone()));
                        }

                        // restore memorised bindings for all composite arguments
                        std::vector<const Argument*> compositeArguments;
                        visitDepthFirst(*magicClause, [&](const Argument& argument) {
                        ...
                        for (const Argument* compositeArgument : compositeArguments) {
                            ...
                            if (compositeBindings.isVariableBoundComposite(argName)) {
                                ...
                                magicClause->addToBody(
                                        std::make_unique<BinaryConstraint>(BinaryConstraintOp::EQ,
                                                std::unique_ptr<Argument>(compositeArgument->clone()),
                                                std::unique_ptr<Argument>(originalArgument)));
                            }
                        }

                        // restore bindings for normalised constants
                        std::vector<const Variable*> clauseVariables;
                        visitDepthFirst(*magicClause,
                                [&](const Variable& variable) { clauseVariables.push_back(&variable); });

                        for (const Variable* var : clauseVariables) {
                            std::string varName = getString(var);

                            // all normalised constants begin with "+abdul" (see Transforms.cpp)
                            // +abdulX_variablevalue_Y
                            if (hasPrefix(varName, "+abdul")) {
                                Argument* embeddedConstant =
                                        extractConstant(translationUnit.getSymbolTable(), varName);

                                // add the constraint to the body of the clause
                                magicClause->addToBody(std::make_unique<BinaryConstraint>(
                                        BinaryConstraintOp::EQ, std::unique_ptr<Argument>(var->clone()),
                                        std::unique_ptr<Argument>(embeddedConstant)));
                            }
                        }

                        // magic rule done! add it to the program
                        program->appendClause(std::unique_ptr<Clause>(magicClause));
                    }
                }
            }
            ...
            const Atom* newClauseHead = newClause->getHead()->getAtom();
            RelationIdentifier newMag = createMagicIdentifier(newClauseHead->getName(), querynum);
            Atom* newMagAtom = new Atom(newMag);

            // copy over the bound arguments from the head
            std::vector<Argument*> args = newClauseHead->getArguments();
            for (size_t k = 0; k < args.size(); k++) {
                if (headAdornment[k] == 'b') {
                    newMagAtom->addArgument(std::unique_ptr<Argument>(args[k]->clone()));
                }
            }

            // add it to the end of the clause
            newClause->addToBody(std::unique_ptr<Atom>(newMagAtom));
            ...
            adornedRelation->addClause(std::unique_ptr<Clause>(newClause));
        }
    }

    // remove all transformed old IDB relations, making sure to preserve input/output directives
    for (RelationIdentifier relationName : oldIdb) {
        Relation* relation = program->getRelation(relationName);
        ...
        if (relation->isOutput()) {
            ...
            std::vector<IODirective*> clonedDirectives;
            for (IODirective* iodir : relation->getIODirectives()) {
    ...
    for (size_t i = 0; i < outputQueries.size(); i++) {
        ...
        if (outputRelation == nullptr) {
            ...
            IODirective* newdir = new IODirective();
            if (addAsOutput.find(oldName) != addAsOutput.end()) {
                newdir->setAsOutput();
            } else {
                newdir->setAsPrintSize();
            }
            outputRelation->addIODirectives(std::unique_ptr<IODirective>(newdir));

            // add the new output to the program
            program->appendRelation(std::unique_ptr<Relation>(outputRelation));
        }

        // rules need to be the same
        // easy fix:
        //    oldname(arg1...argn) :- newname(arg1...argn)
        Atom* headatom = new Atom(oldName);
        Atom* bodyatom = new Atom(newRelationName);
        ...
        Clause* referringClause = new Clause();
        referringClause->setSrcLoc(nextSrcLoc(outputRelation->getSrcLoc()));
        referringClause->setHead(std::unique_ptr<Atom>(headatom));
        referringClause->addToBody(std::unique_ptr<Atom>(bodyatom));

        program->appendClause(std::unique_ptr<Clause>(referringClause));
    }

    // add in all the output directives to their corresponding relations
    for (std::pair<RelationIdentifier, std::vector<IODirective*>> iopair : outputDirectives) {
        for (IODirective* iodir : iopair.second) {
            program->getRelation(iopair.first)->addIODirectives(std::unique_ptr<IODirective>(iodir));
        }
    }
