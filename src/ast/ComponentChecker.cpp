...
const Component* ComponentChecker::checkComponentNameReference(ErrorReport& report,
        const Component* enclosingComponent, const ComponentLookup& componentLookup, const std::string& name,
        const SrcLocation& loc, const TypeBinding& binding) {
    const TypeIdentifier& forwarded = binding.find(name);
    if (!forwarded.empty()) {
...
void ComponentChecker::checkComponent(ErrorReport& report, const Component* enclosingComponent,
    ...
    std::vector<TypeIdentifier> actualParams(parentTypeParameters.size(), "<type parameter>");
