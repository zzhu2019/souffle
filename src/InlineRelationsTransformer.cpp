...
// Essentially, produce every combination (m_1 ^ m_2 ^ ...) where m_i is the negation of a literal
// in the ith rule of a.
    ...
    if(const AstUnaryFunctor* functor = dynamic_cast<const AstUnaryFunctor*>(arg)) {
      ...
      if(leftVersions.isValid()) {
        ...
        if(middleVersions.isValid()) {
          ...
          NullableVector<AstArgument*> rightVersions = getInlinedArgument(program, functor->getArg(2));
