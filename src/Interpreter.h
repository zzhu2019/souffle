...
public:
    InterpreterRelation(size_t relArity, bool eqRel)
            : arity(relArity), eqRel(eqRel), num_tuples(0), head(std::unique_ptr<Block>(new Block())), tail(head.get()),
              totalIndex(nullptr) {}

    InterpreterRelation(const InterpreterRelation& other) = delete;

    InterpreterRelation(InterpreterRelation&& other)
            : arity(other.arity), eqRel(other.eqRel), num_tuples(other.num_tuples), tail(other.tail),
...
public:
    ...
    InterpreterRelation& getRelation(const RamRelation& id) {
        ...
        if (pos != data.end()) {
            ...
            res = &(data.emplace(id.getName(), InterpreterRelation(id.getArity(),id.isEqRel())).first->second);
        }
