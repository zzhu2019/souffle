...
#  define NL                    1
#  define EL                    0
#  define IS_LOCKED(lock)       (__atomic_load_n((long int *) &fallback_lock,__ATOMIC_SEQ_CST) != fallback_unlocked_value)

#  define TX_RETRIES(num)   int retries = num; 
#  define TX_START(type)    while (1) { \
                        while (IS_LOCKED(fallback_lock)); \
                        unsigned status = _xbegin(); \
                        if (status == _XBEGIN_STARTED) { \
                            if (IS_LOCKED(fallback_lock)) \
                                _xabort(1); \
                            break; \
                        } else { \
...
#  define TX_START_INST(type, data)    while (1) { \
                        while (IS_LOCKED(fallback_lock)); \
