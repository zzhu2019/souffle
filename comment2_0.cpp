...
namespace souffle {
using namespace ram;
class Sf_comment2_0 : public SouffleProgram {
private:
...
// -- Table: B
ram::Relation<Auto,1>* rel_1_B;
souffle::RelationWrapper<0,ram::Relation<Auto,1>,Tuple<RamDomain,1>,1,false,true> wrapper_rel_1_B;
public:
Sf_comment2_0() : rel_1_B(new ram::Relation<Auto,1>()),
wrapper_rel_1_B(*rel_1_B,symTable,"B",std::array<const char *,1>{"i:number"},std::array<const char *,1>{"x"}){
addRelation("B",&wrapper_rel_1_B,0,1);
}
~Sf_comment2_0() {
delete rel_1_B;
}
...
if (performIO) {
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"filename","/home/lyndonhenry/Dropbox/workspace/souffle/tests/testsuite.dir/syntactic/comment2/id/B.csv"},{"name","B"}});
if (!outputDirectory.empty() && directiveMap["IO"] == "file" && directiveMap["filename"].front() != '/') {directiveMap["filename"] = outputDirectory + "/" + directiveMap["filename"];}
IODirectives ioDirectives(directiveMap);
IOSystem::getInstance().getWriter(SymbolMask({0}), symTable, ioDirectives, 0)->writeAll(*rel_1_B);
} catch (std::exception& e) {std::cerr << e.what();exit(1);}
}
/* END visitStore @RamExecutor.cpp:1283 */
/* BEGIN visitDrop @RamExecutor.cpp:1397 */
if (performIO || 0) rel_1_B->purge();
...
SouffleProgram *newInstance_comment2(){return new Sf_comment2_0;}
SymbolTable *getST_comment2(SouffleProgram *p){return &reinterpret_cast<Sf_comment2_0*>(p)->symTable;}
#ifdef __EMBEDDED_SOUFFLE__
class factory_Sf_comment2_0: public souffle::ProgramFactory {
SouffleProgram *newInstance() {
return new Sf_comment2_0();
};
public:
factory_Sf_comment2_0() : ProgramFactory("comment2"){}
};
static factory_Sf_comment2_0 __factory_Sf_comment2_0_instance;
}
#else
}
