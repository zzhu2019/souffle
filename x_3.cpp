...
namespace souffle {
using namespace ram;
class Sf_x_3 : public SouffleProgram {
private:
...
// -- Table: r_band
ram::Relation<Auto,3>* rel_5_r_band;
souffle::RelationWrapper<1,ram::Relation<Auto,3>,Tuple<RamDomain,3>,3,false,true> wrapper_rel_5_r_band;
public:
Sf_x_3() : rel_1_A(new ram::Relation<Auto,1>()),
wrapper_rel_1_A(*rel_1_A,symTable,"A",std::array<const char *,1>{"i:number"},std::array<const char *,1>{"x"}),
rel_5_r_band(new ram::Relation<Auto,3>()),
wrapper_rel_5_r_band(*rel_5_r_band,symTable,"r_band",std::array<const char *,3>{"i:number","i:number","i:number"},std::array<const char *,3>{"x","y","z"}){
addRelation("A",&wrapper_rel_1_A,1,0);
addRelation("r_band",&wrapper_rel_5_r_band,0,1);
}
~Sf_x_3() {
delete rel_1_A;
delete rel_5_r_band;
...
SignalHandler::instance()->setMsg(R"_(r_band((xbandy),x,y) :- 
   A(x),
   A(y).
in file /home/lyndonhenry/Dropbox/workspace/souffle/tests/semantic/bitwise/bitwise.dl [9:1-9:38])_");
/* BEGIN visitInsert @RamExecutor.cpp:1287 */
if (!rel_1_A->empty()) {
auto part = rel_1_A->partition();
PARALLEL_START;
CREATE_OP_CONTEXT(rel_1_A_op_ctxt,rel_1_A->createContext());
CREATE_OP_CONTEXT(rel_5_r_band_op_ctxt,rel_5_r_band->createContext());
...
rel_5_r_band->insert(tuple,READ_OP_CONTEXT(rel_5_r_band_op_ctxt));
...
if (performIO) {
try {std::map<std::string, std::string> directiveMap({{"IO","file"},{"filename","/home/lyndonhenry/Dropbox/workspace/souffle/tests/testsuite.dir/semantic/bitwise/id/r_band.csv"},{"name","r_band"}});
if (!outputDirectory.empty() && directiveMap["IO"] == "file" && directiveMap["filename"].front() != '/') {directiveMap["filename"] = outputDirectory + "/" + directiveMap["filename"];}
IODirectives ioDirectives(directiveMap);
IOSystem::getInstance().getWriter(SymbolMask({0, 0, 0}), symTable, ioDirectives, 0)->writeAll(*rel_5_r_band);
} catch (std::exception& e) {std::cerr << e.what();exit(1);}
}
/* END visitStore @RamExecutor.cpp:1283 */
/* BEGIN visitDrop @RamExecutor.cpp:1397 */
if (performIO || 0) rel_5_r_band->purge();
/* END visitDrop @RamExecutor.cpp:1401 */
...
std::cout << "\n";
std::cout << "Relation rel_5_r_band:\n";
rel_5_r_band->printHintStatistics(std::cout,"  ");
std::cout << "\n";
}
}
public:
...
try {IODirectives ioDirectives;
ioDirectives.setIOType("stdout");
ioDirectives.setRelationName("rel_5_r_band");
IOSystem::getInstance().getWriter(SymbolMask({0, 0, 0}), symTable, ioDirectives, 0)->writeAll(*rel_5_r_band);
} catch (std::exception& e) {std::cerr << e.what();exit(1);}
}
public:
...
SouffleProgram *newInstance_x(){return new Sf_x_3;}
SymbolTable *getST_x(SouffleProgram *p){return &reinterpret_cast<Sf_x_3*>(p)->symTable;}
#ifdef __EMBEDDED_SOUFFLE__
class factory_Sf_x_3: public souffle::ProgramFactory {
SouffleProgram *newInstance() {
return new Sf_x_3();
};
public:
factory_Sf_x_3() : ProgramFactory("x"){}
};
static factory_Sf_x_3 __factory_Sf_x_3_instance;
}
#else
}
